...
const BASE_METRICS = [
  ...
  { label: "PULSE AMP", value: "0.72 V" },
  { label: "PTT", value: "245 ms" },
  { label: "RISE TIME", value: "165 ms" },
];

function SensorBadge({ isConnected }: { isConnected: boolean }) {
  return (
    <span
      data-ocid="ppg-sensor-badge"
...
export default function PPGWaveform() {
  ...
  const ppgStatus = useAppStore((s) => s.sensorStatus.ppg);
  const isConnected = ppgStatus === "CONNECTED";
